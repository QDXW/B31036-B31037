...
* 范    例:
******************************************************************************/
void YcReport(uint8_t uRelNum,uint16_t uIecAddr,uint32_t fIecVal,uint8_t uStep)
{
    Uint_Char_Convert  uCTemp;
    ...
    do
    {
        if(g_sBurst.yc_count<30 &&  (1==uStep || 3==uStep)) // 一帧104报文，最多可以容纳30个遥测点
        {
            ...
            //***********************************突发极大值置位全F值************            
            if(((0x4F800000==yc_temp.u)&&(g_pRegPoint[uRelNum][uCTemp.u].reg_type.type.data == T_UINT32)) ||
                ((0x477FFF00==yc_temp.u)&&(g_pRegPoint[uRelNum][uCTemp.u].reg_type.type.data == T_UINT16)))
            {
        ...
        case TYPE_YC:  // 发送查询遥测
            ...
            for(i=0; i<uRecBuffer[2];)
            {
                ...
                if(IEC104_DATA_YC[uIecAddr] != uYcU.u)//
                {
                    ...
                    YcReport(uRelNum,uIecAddr,uYcU.u,1); // 突发遥测数据
                }
...
uint8_t SouthReadSD(void)
{
    ...
    */
	...
	for(uRelNum = 1; uRelNum < MAX_device+1; uRelNum++)
	{
		if(g_DeviceSouth.protocol[uRelNum].mess_point_sum != 0)
		{
			for(j = 0; j < g_DeviceSouth.protocol[uRelNum].mess_point_sum; j++)	//搜索MODBUS地址
			{
				if(TYPE_SD == g_pRegPoint[uRelNum][j].reg_type.type.mess)
				{
					for(i = 0;i < MAX_device;i++)
					{
						if(g_DeviceSouth.device_inf[i].rel_num == uRelNum)
						{
							g_DeviceSouth.device_inf[i].reserve++;
						}
					}
				}
			}
    ...
    if(uIecSd)
    {
        for(uRelDevAddr = 0; uRelDevAddr < MAX_device; uRelDevAddr++)	//遍历设备
        {
            ...
            if((g_DeviceSouth.device_inf[uRelDevAddr].sd_start_addr <= uIecSd) 
				&& (uIecSd < (g_DeviceSouth.device_inf[uRelDevAddr].sd_start_addr + g_DeviceSouth.device_inf[uRelDevAddr].reserve)))
            {
                uSdPoint = uIecSd - g_DeviceSouth.device_inf[uRelDevAddr].sd_start_addr + 1;
                break;
            }
