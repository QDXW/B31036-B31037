...
int16_t ComMasterWrite(MODBUS_MASTER_T *pMaster,uint8_t uAddr,uint8_t uCmd,uint16_t uRegAddr,uint16_t uQty,const uint16_t *pReg)
{
    ...
    do
    {
        ...
        if(iRecCount<=0)
        {
            ...
            if(TimeGapJudge(uFrameSendTime,sNowTime,(0x10==uCmd)?800:pMaster->uFailTimeOut))//if(((sNowTime.tv_sec - uFrameSendTime.tv_sec)*1000000 + sNowTime.tv_usec-uFrameSendTime.tv_usec)<(pMaster->uFailTimeOut*1000))
            {
                msleep(5);
                continue;
            }
...
int16_t ComMasterRead(MODBUS_MASTER_T *pMaster,uint8_t uAddr,uint8_t uCmd,uint16_t uRegAddr,uint16_t uQty,uint8_t *pRecData,uint8_t *pSendData)
{
    ...
    do
    {
        ...
        if((iRecCount<=0))
        {
            ...
            if(TimeGapJudge(uFrameSendTime,sNowTime,(uCmd == 0x03)?(pMaster->uFailTimeOut * 6):pMaster->uFailTimeOut))
			{
				msleep(20);				 
				continue;
			}
